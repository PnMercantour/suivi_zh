...
function(feature, context) {
    if (feature.properties.id == context.props.hideout.id)
        return {color: 'red', weight: 5, fillOpacity: 0.4};
    return {color:'green', fillOpacity: 0.4};
}""")

valleeHoverStyle = arrow_function(
    dict(weight=5, fillOpacity=0))

valleeLayer = dl.GeoJSON(
    url=app.get_asset_url('vallees.json'),
    id="valleeLayer",
    hideout={'id': None},
    # hoverStyle is broken (changes z-index)
    # hoverStyle=valleeHoverStyle,
    options=dict(
        style=valleeStyle,
        pane="overlayPane",
...
sitePointToLayer = assign("""function(feature, latlng, context){
    if (context.props.hideout) {
        if (feature.properties.id == context.props.hideout.id) {
            circleOptions= {color: "yellow", fillOpacity: 0.8};
        } else if (feature.properties.id_vallee == context.props.hideout.id_vallee) {
            circleOptions= {color: "white", fillColor: "blue"};
        } else {
...
siteHoverStyle = arrow_function(
    dict(weight=5, fillOpacity=0))

siteLayer = dl.GeoJSON(
    url=app.get_asset_url('sites.json'),
    id="siteLayer",
    hoverStyle=siteHoverStyle,
...
""",
    Output("valleeLayer", "hideout"), Output("valleeLayer", 'click_feature'),
