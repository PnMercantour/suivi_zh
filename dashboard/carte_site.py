...
map = dl.Map(
    dl.LayersControl(
        [
            ...
            dl.BaseLayer(
                tile.ign("orthohisto", "2011-2015"), name="Vue 2011-2015", checked=False
            ),
            dl.BaseLayer(
                tile.ign("orthohisto", "2006-2010"), name="Vue 2006-2010", checked=False
            ),
            dl.BaseLayer(
                tile.ign("orthohisto", "2000-2005"), name="Vue 2000-2005", checked=False
            ),
            dl.BaseLayer(
                tile.ign("pngorthohisto", ".1950-1965"),
                name="Vue 1950 - 1965",
                checked=False,
            ),
            dl.Pane(
            ...
            dl.Overlay(
                dl.Pane(
                    ebf,
                    name="detail_ebf_pane",
                    pane="detail_ebf_pane",
                    style={"zIndex": 455},
                ),
                name="Espaces de bon fonctionnement",
                checked=False,
            ),
...
def process(previous_state, map_click, vallee, site, zh, rhomeo):
    if vallee:
        ...
        return {
            "vallee": (
                clicked_vallee
                if (clicked_vallee != prev_vallee or prev_site is not None)
                else None
            ),
            "site": (
                prev_site
                if (clicked_vallee == prev_vallee and previous_state["zh"] is not None)
                else None
            ),
            "zh": None,
        }
...
def make_title(zh=None, site=None, vallee=None):
    ...
    return (
        f"{data.site_data[site]['nom_site']} ({data.vallee_data[vallee]['nom_vallee']})"
    )
...
def update(new_state, old_state, force_update):
    ...
    return {
        ...
        "rhomeo_hideout": {
            "site": site,
            "vallee": vallee,
            "code": data.site_data[site]["rhomeo"] if site is not None else None,
        },
        "title": (
