...
app.layout = html.Div([html.Div(dcc.Graph(id='map', clickData={'points': [{'hovertext': 'Japan'}]})),
            ...
            html.Div([dcc.Dropdown(id="dropdown1", options=[{'label': i, 'value': i} for i in ['lifeExp', 'pop', 'gdpPercap']]), 
            dcc.Graph(id='graph1'), 
            dcc.Dropdown(id="dropdown2", options=[{'label': i, 'value': i} for i in ['lifeExp', 'pop', 'gdpPercap']]), 
            dcc.Graph(id='graph2')])
            ])
...
def display_graph1(clickValue, type):
...
def display_graph2(clickValue, type):
    dff = df[df['country']==clickValue['points'][0]['hovertext']]
    fig = px.scatter(x=dff['year'], y=dff[str(type)])
    return fig
